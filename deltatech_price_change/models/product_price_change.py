...
class ProductPriceChange(models.Model):
    ...
    date = fields.Date(
        "Date", index=True, states={"done": [("readonly", True)]}, default=fields.Date.today
    )
    ...
    location_id = fields.Many2one("stock.location", "Location",required=True,states={"done": [("readonly", True)]})
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            vals['name'] = self.env['ir.sequence'].next_by_code(
                'price.change') or _('New')
        res = super(ProductPriceChange, self).create(vals)
        return res
